...
// The options are applied to a QueryEngineConfiguration object, which is then used to initialize the Request.
//
...
func WithQueryEngineEngineTaxonomies(engineTaxonomies string) func(*QueryEngineConfiguration) {
	return func(c *QueryEngineConfiguration) {
		engineTaxonomies := parseEngineTaxonomiesArgs(engineTaxonomies)
...
func WithQueryEngineAdvancedRestrictions(advancedRestrictions string) func(*QueryEngineConfiguration) {
	return func(c *QueryEngineConfiguration) {
		restrictions := parseAdvancedRestrictionsArg(advancedRestrictions)
		if len(advancedRestrictions) == 0 {
			return
		}
		c.AdpQueryEngineAdvancedRestrictions = restrictions
	}
}
...
// NewQueryEngineTask returns a new instance of QueryEngineTask.
func NewQueryEngineTask(client *Client, req *Request, executionMode ExecutionMode) *QueryEngineTask {
	return &QueryEngineTask{
		Task: NewTask().WithClient(client).WithRequest(req).WithExecutionMode(executionMode),
	}
}

func (t *QueryEngineTask) GetResultAsString() (string, error) {
	taskResp, err := t.Run()
	if err != nil {
		return "", err
	}
