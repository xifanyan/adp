...
func NewQueryPostgresqlDBTask(client *Client, req *Request, executionMode ExecutionMode) *QueryPostgresssqlDBTask {
	return &QueryPostgresssqlDBTask{
		Task: NewTask().WithClient(client).WithRequest(req).WithExecutionMode(executionMode),
	}
}

func (t *QueryPostgresssqlDBTask) GetResponseAsString() (string, error) {
	taskResp, err := t.Run()
	if err != nil {
		return "", err
	}
